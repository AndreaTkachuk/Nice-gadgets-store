...
import { getSuggestedItems } from '../../utils/getSuggested';
...
export const PageItem = () => {
  ...
  let suggestedItems = getSuggestedItems([phones, tablets, accessories], 8);

  if (suggestedItems.some(sugItem => sugItem.id === id)) {
    suggestedItems = suggestedItems.filter(it => it.id !== id);
  }
  ...
  return (
    <div className={styles.pageItem}>
      <div className={styles.pageItem__container}>
        ...
        <h2 className={styles.pageItem__alsoLike__title}>You may also like</h2>
        <Carousel items={suggestedItems} visibleDiscount={true} />
      </div>
    </div>
  );
};
